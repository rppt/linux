...
static int alloc_raw_top_down_simple_check(void)
{
	...
	allocated_ptr = memblock_alloc_raw(size, SMP_CACHE_BYTES);
 ...
 */
static int alloc_raw_top_down_disjoint_check(void)
{
	...
	allocated_ptr = memblock_alloc_raw(r2_size, alignment);
 ...
 */
static int alloc_raw_top_down_before_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(r2_size, SMP_CACHE_BYTES);
 ...
 * get updated.
 */
static int alloc_raw_top_down_after_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	struct region r1;
	void *allocated_ptr = NULL;
 ...
 * updated.
 */
static int alloc_raw_top_down_second_fit_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	struct region r1, r2;
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(r3_size, SMP_CACHE_BYTES);
 ...
 */
static int alloc_raw_in_between_generic_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	struct region r1, r2;
	void *allocated_ptr = NULL;
 ...
 */
static int alloc_raw_small_gaps_generic_check(void)
{
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(region_size, SMP_CACHE_BYTES);
 ...
 * Expect no allocation to happen.
 */
static int alloc_raw_all_reserved_generic_check(void)
{
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(SZ_256, SMP_CACHE_BYTES);
 ...
 * Expect no allocation to happen.
 */
static int alloc_raw_no_space_generic_check(void)
{
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(SZ_1K, SMP_CACHE_BYTES);
 ...
 * gets updated.
 */
static int alloc_raw_limited_space_generic_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(available_size, SMP_CACHE_BYTES);
 ...
 * Expect no allocation to happen.
 */
static int alloc_raw_no_memory_generic_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
 ...
 * memory.
 */
static int alloc_raw_bottom_up_simple_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(SZ_2, SMP_CACHE_BYTES);
 ...
 * Expect no allocation to happen.
 */
static int alloc_raw_too_large_generic_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(MEM_SIZE + SZ_2, SMP_CACHE_BYTES);
 ...
 */
static int alloc_raw_bottom_up_disjoint_check(void)
{
 ...
 * Expect a merge of both regions. Only the region size gets updated.
 */
static int alloc_raw_bottom_up_before_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	void *allocated_ptr = NULL;
	...
	allocated_ptr = memblock_alloc_raw(r1_size, SMP_CACHE_BYTES);
 ...
 * Expect a merge of both regions. Only the region size gets updated.
 */
static int alloc_raw_bottom_up_after_check(void)
{
	struct memblock_region *rgn = &memblock.reserved.regions[0];
	struct region r1;
	void *allocated_ptr = NULL;
 ...
 * get updated.
 */
static int alloc_raw_bottom_up_second_fit_check(void)
{
...
/* Test case wrappers */
static int alloc_raw_simple_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	alloc_raw_top_down_simple_check();
	memblock_set_bottom_up(true);
	alloc_raw_bottom_up_simple_check();

	return 0;
}

static int alloc_raw_disjoint_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	alloc_raw_top_down_disjoint_check();
	memblock_set_bottom_up(true);
	alloc_raw_bottom_up_disjoint_check();

	return 0;
}

static int alloc_raw_before_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	alloc_raw_top_down_before_check();
	memblock_set_bottom_up(true);
	alloc_raw_bottom_up_before_check();

	return 0;
}

static int alloc_raw_after_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	alloc_raw_top_down_after_check();
	memblock_set_bottom_up(true);
	alloc_raw_bottom_up_after_check();

	return 0;
}

static int alloc_raw_in_between_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_in_between_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_in_between_generic_check);

	return 0;
}

static int alloc_raw_second_fit_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	alloc_raw_top_down_second_fit_check();
	memblock_set_bottom_up(true);
	alloc_raw_bottom_up_second_fit_check();

	return 0;
}

static int alloc_raw_small_gaps_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_small_gaps_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_small_gaps_generic_check);

	return 0;
}

static int alloc_raw_all_reserved_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_all_reserved_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_all_reserved_generic_check);

	return 0;
}

static int alloc_raw_no_space_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_no_space_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_no_space_generic_check);

	return 0;
}

static int alloc_raw_limited_space_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_limited_space_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_limited_space_generic_check);

	return 0;
}

static int alloc_raw_no_memory_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_no_memory_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_no_memory_generic_check);

	return 0;
}

static int alloc_raw_too_large_check(void)
{
	test_print("\tRunning %s...\n", __func__);
	memblock_set_bottom_up(false);
	top_down_run(alloc_raw_too_large_generic_check);
	memblock_set_bottom_up(true);
	bottom_up_run(alloc_raw_too_large_generic_check);

	return 0;
}

int memblock_alloc_raw_checks(void)
{
	const char *func_testing = "memblock_alloc_raw";
	...
	alloc_raw_simple_check();
	alloc_raw_disjoint_check();
	alloc_raw_before_check();
	alloc_raw_after_check();
	alloc_raw_second_fit_check();
	alloc_raw_small_gaps_check();
	alloc_raw_in_between_check();
	alloc_raw_all_reserved_check();
	alloc_raw_no_space_check();
	alloc_raw_limited_space_check();
	alloc_raw_no_memory_check();
	alloc_raw_too_large_check();
