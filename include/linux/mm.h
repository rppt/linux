...
#endif
	/* Bits 41 and 43 are free (were UFFD_MINOR and UFFD_RWP) */
 ...
 * source: userfaultfd context may be cleared on the destination
 ...
 *                           fault.
 *
 * Userfaultfd WP/RWP modes also require page table copying because they
 * encode metadata about installed uffd write- or read-write-protect
 * handlers, which cannot be reconstructed on page fault.  This is checked
 * separately via userfaultfd_protected() in vma_needs_copy().
 */
...
{
	/*
	 * Userfaultfd RWP uses protnone as an access-tracking marker, not
	 * for NUMA hinting. GUP must always take a fault so the access is
	 ...
	 */
#ifdef CONFIG_USERFAULTFD
	if (vma_is_accessible(vma) && (vma->vm_flags & VM_UFFD) &&
