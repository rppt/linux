...
/*
 * free_text_area for vmalloc_execmem()
 */
 ...
 * Return: pointer to the allocated memory or %NULL on error
 */
void *vmalloc_execmem(unsigned long size, unsigned long align)
{
...
/**
 * execmem_fill - Copy text to RO+X memory allocated by vmalloc_execmem()
 * @dst:  pointer to memory allocated by vmalloc_execmem()
...
void *execmem_fill(void *dst, void *src, size_t len)
{
	...
	/* Disallow filling across vmalloc_execmem boundaries */
...
/**
 * vfree_execmem - Release memory allocated by vmalloc_execmem()
 * @addr:  Memory base address
 *
 * If @addr is NULL, no operation is performed.
 */
void vfree_execmem(void *addr)
{
	 ...
	 * after one vfree_execmem(). But we free at most one vm for now.
	 */
