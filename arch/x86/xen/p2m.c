...
static void __init xen_rebuild_p2m_list(unsigned long *p2m)
{
	...
	paravirt_alloc_pte(&init_mm.pgt, __pa(p2m_missing_pte) >> PAGE_SHIFT);
	p2m_identity_pte = alloc_p2m_page();
	paravirt_alloc_pte(&init_mm.pgt, __pa(p2m_identity_pte) >> PAGE_SHIFT);
...
static pte_t *alloc_p2m_pmd(unsigned long addr, pte_t *pte_pg)
{
	...
	for (i = 0; i < PMDS_PER_MID_PAGE; i++) {
		...
		paravirt_alloc_pte(&init_mm.pgt, __pa(pte_newpg[i]) >> PAGE_SHIFT);
