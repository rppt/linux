				     ...
				     struct module *mod)
{
	...
	for (p = start; p < end; p++) {
		...
		wr_addr = p->instr + module_writable_offset(mod, p->instr);
		text_poke_early(wr_addr, insn_buff, p->len);
	}
}
...
void __init alternative_instructions(void)
{
	 ...
	 */
	apply_paravirt(__parainstructions, __parainstructions_end, NULL);
